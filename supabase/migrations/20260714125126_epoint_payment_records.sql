...
create index payment_webhook_events_merchant_order_id_idx
  on public.payment_webhook_events (merchant_order_id);
...
begin
  ...
  set amount = case
        ...
        else payment_transactions.amount
      end,
      currency = case
        when excluded.status in ('success', 'returned') then excluded.currency
        else payment_transactions.currency
      end,
      status = case
        ...
        end) >= (case payment_transactions.status
          when 'new' then 10
          when 'error' then 20
          when 'server_error' then 30
          when 'success' then 40
        ...
        else payment_transactions.status
      end,
      provider_transaction_id = coalesce(excluded.provider_transaction_id, payment_transactions.provider_transaction_id),
      bank_transaction_id = coalesce(excluded.bank_transaction_id, payment_transactions.bank_transaction_id),
      operation_code = coalesce(excluded.operation_code, payment_transactions.operation_code),
      response_code = coalesce(excluded.response_code, payment_transactions.response_code),
      rrn = coalesce(excluded.rrn, payment_transactions.rrn),
      card_mask = coalesce(excluded.card_mask, payment_transactions.card_mask),
      message = coalesce(excluded.message, payment_transactions.message),
      last_callback_payload = excluded.last_callback_payload,
      paid_at = case
        when excluded.status = 'success' then coalesce(payment_transactions.paid_at, now())
        else payment_transactions.paid_at
      end,
      returned_at = case
        when excluded.status = 'returned' then coalesce(payment_transactions.returned_at, now())
        else payment_transactions.returned_at
      end,
      updated_at = now();
