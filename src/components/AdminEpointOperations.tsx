...
export function AdminEpointOperations() {
  ...
  const load = useCallback(async () => {
    ...
    if (opsResult.error || cardsResult.error || settingsResult.error) {
      toast.error(opsResult.error?.message || cardsResult.error?.message || settingsResult.error?.message || "Epoint mÉ™lumatlarÄ± alÄ±nmadÄ±");
    } else {
  ...
  const invoke = async (body: Record<string, unknown>, success: string) => {
    ...
    setWorking(false);
    if (error || data?.error) {
      toast.error(data?.error || error?.message || "Epoint É™mÉ™liyyatÄ± alÄ±nmadÄ±");
      return null;
    }
  ...
  const saveSplit = async () => {
    ...
    if (!Number.isFinite(percent) || percent < 0 || percent > 100) return toast.error("Faiz 0-100 aralÄ±ÄŸÄ±nda olmalÄ±dÄ±r");
    ...
    if (error) toast.error(error.message); else toast.success("Split ayarlarÄ± saxlanÄ±ldÄ±");
  };
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <h2 className="text-xl font-bold">Epoint idarÉ™etmÉ™si</h2>
          <p className="text-sm text-muted-foreground">Kart tokenlÉ™ri, Ã¶dÉ™niÅŸ statuslarÄ±, qaytarmalar vÉ™ audit</p>
        </div>
        <button type="button" onClick={() => { void load(); void checkHeartbeat(); }} className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium hover:bg-muted" title="YenilÉ™">
          <RefreshCw className="h-4 w-4" /> YenilÉ™
        </button>
      </div>

      <div className="grid gap-3 sm:grid-cols-3">
        <Metric icon={Activity} label="Epoint xidmÉ™ti" value={heartbeat?.ok ? `Aktiv${heartbeat.latency_ms ? ` Â· ${heartbeat.latency_ms} ms` : ""}` : "ÆlÃ§atan deyil"} good={Boolean(heartbeat?.ok)} />
        <Metric icon={CreditCard} label="Aktiv saxlanmÄ±ÅŸ kart" value={String(cards.filter((card) => card.status === "active").length)} good />
        <Metric icon={ShieldCheck} label="Son 100 É™mÉ™liyyat" value={String(operations.length)} good />
      </div>

      <section className="border-y border-border py-5">
        <div className="mb-4 flex items-center gap-2"><Split className="h-5 w-5 text-primary" /><h3 className="font-semibold">Split Ã¶dÉ™niÅŸ ayarlarÄ±</h3></div>
        <div className="grid max-w-3xl gap-3 sm:grid-cols-[1fr_160px_auto]">
          <label className="text-sm"><span className="mb-1 block text-muted-foreground">Ä°kinci Epoint istifadÉ™Ã§isi</span><input value={splitUser} onChange={(event) => setSplitUser(event.target.value)} className="h-10 w-full rounded-md border border-border bg-background px-3" placeholder="i000000002" /></label>
      ...
      <section className="border-b border-border pb-5">
        <h3 className="mb-4 font-semibold">MaliyyÉ™ É™mÉ™liyyatlarÄ±</h3>
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              <button disabled={working || !paymentIntentId} type="button" onClick={() => void invoke({ action: "get_status", payment_intent_id: paymentIntentId }, "Status Epoint-dÉ™n yenilÉ™ndi")} className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium disabled:opacity-50"><Search className="h-4 w-4" /> Statusu yoxla</button>
              <input aria-label="Qaytarma mÉ™blÉ™ÄŸi" value={reverseAmount} onChange={(event) => setReverseAmount(event.target.value)} className="h-10 w-32 rounded-md border border-border bg-background px-3" placeholder="Tam mÉ™blÉ™ÄŸ" />
              <button disabled={working || !paymentIntentId} type="button" onClick={() => void invoke({ action: "reverse", payment_intent_id: paymentIntentId, ...(reverseAmount ? { amount: Number(reverseAmount) } : {}) }, "Qaytarma sorÄŸusu icra edildi")} className="inline-flex h-10 items-center gap-2 rounded-md bg-destructive px-3 text-sm font-semibold text-destructive-foreground disabled:opacity-50"><RotateCcw className="h-4 w-4" /> Geri qaytar</button>
            </div>
          </div>
          <div className="space-y-3">
            ...
            <button disabled={working || !invoiceId} type="button" onClick={() => void invoke({ action: "invoice_view", invoice: { id: Number(invoiceId) } }, "Hesab-faktura mÉ™lumatÄ± alÄ±ndÄ±")} className="inline-flex h-10 items-center gap-2 rounded-md border border-border px-3 text-sm font-medium disabled:opacity-50"><FileText className="h-4 w-4" /> Fakturaya bax</button>
          </div>
        </div>
      </section>
      ...
      <section>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <h3 className="font-semibold">ÆmÉ™liyyat jurnalÄ±</h3>
          <div className="relative w-full sm:w-72"><Search className="absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" /><input value={filter} onChange={(event) => setFilter(event.target.value)} className="h-10 w-full rounded-md border border-border bg-background pl-9 pr-3 text-sm" placeholder="ÆmÉ™liyyat axtar" /></div>
        </div>
        <div className="overflow-x-auto border-y border-border">
          <table className="w-full min-w-[760px] text-left text-sm">
            <thead className="bg-muted/50 text-xs text-muted-foreground"><tr><th className="px-3 py-2">NÃ¶v</th><th className="px-3 py-2">Status</th><th className="px-3 py-2">MÉ™blÉ™ÄŸ</th><th className="px-3 py-2">Payment ID</th><th className="px-3 py-2">Tarix</th></tr></thead>
            <tbody>{loading ? <tr><td colSpan={5} className="px-3 py-8 text-center text-muted-foreground">YÃ¼klÉ™nir...</td></tr> : visible.length === 0 ? <tr><td colSpan={5} className="px-3 py-8 text-center text-muted-foreground">ÆmÉ™liyyat yoxdur</td></tr> : visible.map((item) => (
              <tr key={item.id} className="border-t border-border"><td className="px-3 py-2 font-medium">{item.kind}</td><td className="px-3 py-2"><Status value={item.status} /></td><td className="px-3 py-2">{item.amount == null ? "â€”" : formatAZN(item.amount)}</td><td className="max-w-52 truncate px-3 py-2 font-mono text-xs" title={item.payment_intent_id ?? ""}>{item.payment_intent_id ?? "â€”"}</td><td className="px-3 py-2 text-xs">{formatDate(item.created_at)}</td></tr>
            ))}</tbody>
          </table>
        </div>
      </section>
