    ...
    <div className="space-y-4 max-w-2xl">
      <div className="bg-card border border-border rounded-2xl p-5 space-y-4">
        ...
        <div className="rounded-xl border border-border p-4 space-y-3">
          <div className="font-bold">Satıcı telefon təsdiqi</div>
          <div className="flex items-center justify-between gap-4">
            <div>
              <div className="font-semibold text-sm">SMS/OTP təsdiqi</div>
              <div className="text-xs text-muted-foreground">
                Aktiv olduqda satıcı SMS kodunu təsdiqləmədən qeydiyyat ödənişinə keçə bilməz.
                Deaktiv etmək Twilio məlumatlarını silmir.
              </div>
            </div>
            <button
              onClick={() => updateSettings({ seller_phone_otp_required: !settings.seller_phone_otp_required })}
              className={`shrink-0 px-4 py-2 rounded-lg font-bold text-sm ${
                settings.seller_phone_otp_required
                  ? "bg-success text-white"
                  : "bg-secondary text-muted-foreground"
              }`}
            >
              {settings.seller_phone_otp_required ? "Aktiv" : "Deaktiv"}
            </button>
          </div>
          {!settings.seller_phone_otp_required && (
            <div className="rounded-lg border border-amber-300 bg-amber-50 px-3 py-2 text-xs text-amber-900">
              SMS/OTP deaktivdir. Yeni satıcılar telefon kodu olmadan ödəniş mərhələsinə keçə biləcək.
            </div>
          )}
        </div>
        <div className="rounded-xl border border-border p-4 space-y-3">
