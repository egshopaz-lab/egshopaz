...
export function AdminPayouts() {
  ...
  const load = async () => {
    ...
    const [r, b] = await Promise.all([
      ...
      supabase
        ...
        .select("seller_id, available, pending, total_earned")
    ...
    setRequests(
      (r.data ?? []).map((x: any) => ({
        ...x,
        shop_name: map[x.seller_id]?.shop_name,
        full_name: map[x.seller_id]?.full_name,
      })),
    );
    setBalances(
      (b.data ?? []).map((x: any) => ({
  ...
  const runAutoPayout = async () => {
    ...
    setRunning(true);
    const { data, error } = await (supabase as any).rpc("admin_run_auto_payout");
    setRunning(false);
    if (error) return toast.error(error.message);
  ...
  return (
    <div className="space-y-6">
      <div className="grid sm:grid-cols-3 gap-4">
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="text-xs text-muted-foreground mt-1">
            {requests.filter((r) => r.status === "pending").length} tələb
          </div>
        </div>
        <div className="rounded-xl border border-border bg-card p-5">
        ...
        <div className="rounded-xl border border-border bg-card p-5 flex flex-col justify-between">
          <div>
            ...
            <div className="text-xs text-muted-foreground mt-1">
              Hər gün 02:00-da işləyir. Əl ilə də işə sala bilərsən.
            </div>
          </div>
          <button
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        <div className="flex items-center justify-between mb-3 flex-wrap gap-2">
          <h3 className="font-bold flex items-center gap-2">
            <Wallet className="h-5 w-5 text-primary" /> Çıxarış tələbləri
          </h3>
        ...
        {loading ? (
          ...
          <div className="text-center py-8 text-muted-foreground">Tələb yoxdur</div>
        ) : (
                ...
                <div className="flex items-start justify-between gap-3 flex-wrap">
                  <div className="min-w-0">
                    <div className="font-bold">
                      {r.shop_name || r.full_name || r.seller_id.slice(0, 8)}
                    </div>
                  ...
                  <div className="text-sm">
                    {r.status === "pending" && (
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-yellow-500/15 text-yellow-700 text-xs">
                        <Clock className="h-3 w-3" />
                        Gözləyir
                      </span>
                    )}
                    {r.status === "paid" && (
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-green-500/15 text-green-700 text-xs">
                        <CheckCircle2 className="h-3 w-3" />
                        Ödənilib {r.paid_at ? `· ${formatDateTime(r.paid_at)}` : ""}
                      </span>
                    )}
                    {r.status === "rejected" && (
                      <span className="inline-flex items-center gap-1 px-2 py-1 rounded bg-destructive/15 text-destructive text-xs">
                ...
                <div className="grid sm:grid-cols-2 gap-2 mt-3 text-sm">
                  ...
                  <div className="rounded-md bg-secondary/40 p-2">
                    ...
                    <div className="font-mono flex items-center gap-2 break-all">
                      ...
                      <button
                        onClick={() => copy(r.method === "iban" ? r.iban : r.card_number)}
                    ...
                    {r.bank_name && (
                      <div className="text-xs text-muted-foreground mt-1">{r.bank_name}</div>
                    )}
                  </div>
                </div>
                {r.admin_note && (
                  <div className="text-xs mt-2 text-muted-foreground">📝 {r.admin_note}</div>
                )}
                {r.status === "pending" && (
                  <div className="flex gap-2 mt-3">
                    <button
                      onClick={() => decide(r.id, true)}
                      className="flex-1 h-10 rounded-lg bg-green-600 text-white font-bold text-sm"
                    >
                      ✅ Ödənildi
                    </button>
                    <button
                      onClick={() => decide(r.id, false)}
                      className="flex-1 h-10 rounded-lg bg-destructive text-destructive-foreground font-bold text-sm"
                    >
                      ❌ Rədd et
                    </button>
                  </div>
      ...
      <div className="rounded-xl border border-border bg-card p-5">
        ...
        {balances.length === 0 ? (
          <div className="text-center py-6 text-muted-foreground text-sm">Hələ balans yoxdur</div>
        ) : (
          <div className="space-y-2">
            {balances.map((b) => (
              <div
                key={b.seller_id}
                className="flex items-center justify-between gap-3 pb-2 border-b border-border last:border-0 text-sm"
              >
                <div className="min-w-0 truncate">
                  {b.shop_name || b.full_name || b.seller_id.slice(0, 8)}
                </div>
                <div className="flex gap-4 shrink-0">
                  <div>
                    <span className="text-xs text-muted-foreground">Mövcud:</span>{" "}
                    <b className="text-primary">{formatAZN(b.available)}</b>
                  </div>
                  <div>
                    <span className="text-xs text-muted-foreground">Gözləyir:</span>{" "}
                    <b>{formatAZN(b.pending)}</b>
                  </div>
                  <div className="hidden sm:block">
                    <span className="text-xs text-muted-foreground">Ümumi:</span>{" "}
                    <b>{formatAZN(b.total_earned)}</b>
                  </div>
                </div>
              </div>
            ))}
