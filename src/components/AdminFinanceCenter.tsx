...
export function AdminFinanceCenter({ commissionPercent = 10 }: { commissionPercent?: number }) {
  ...
  const load = useCallback(async () => {
    ...
    const results = await Promise.all([
      db.from("orders").select("id,total,discount,bonus_used,bonus_earned,status,payment_status,payment_method,payment_note,paid_at,created_at,recipient_name,recipient_phone").order("created_at", { ascending: false }).limit(2000),
      ...
      db.from("payout_requests").select("id,seller_id,amount,status,method,iban,card_number,bank_name,account_holder,created_at,paid_at,admin_note").order("created_at", { ascending: false }).limit(2000),
      db.from("epoint_payment_transactions").select("id,merchant_order_id,amount,currency,status,message,provider_transaction_id,bank_transaction_id,response_code,rrn,card_mask,operation_code,paid_at,returned_at,created_at").order("created_at", { ascending: false }).limit(3000),
      ...
      db.from("returns").select("id,order_id,seller_id,refund_amount,status,reason,cost_paid_by,created_at,resolved_at").limit(2000),
  ...
  const report = useMemo(() => {
    ...
    const discounts = money(paidOrders, "discount");
    const bonusUsed = money(paidOrders, "bonus_used");
    const bonusEarned = money(paidOrders, "bonus_earned");
    ...
    return { since, orders, paidOrders, unpaidOrders, pendingPayouts, gross, discounts, bonusUsed, bonusEarned, averageOrder: paidOrders.length ? gross / paidOrders.length : 0, marketplace, registration, advertising, trends, refunds, treasury, cashIn, cashOut, payoutTotal, liability, epoint, success, failed, epointCollected, expectedCollected, discrepancy: epointCollected - expectedCollected, income: marketplace + registration + advertising + trends };
  ...
  const paymentMethods = useMemo(() => {
    ...
    report.paidOrders.forEach((order) => { const key = order.payment_method || "unknown"; const row = map.get(key) ?? { method: key, count: 0, amount: 0 }; row.count += 1; row.amount += Number(order.total || 0); map.set(key, row); });
    return [...map.values()].sort((a, b) => b.amount - a.amount);
  }, [report.paidOrders]);

  const orderStatuses = useMemo(() => {
    const map = new Map<string, { status: string; count: number; amount: number }>();
    report.orders.forEach((order) => { const key = order.status || "unknown"; const row = map.get(key) ?? { status: key, count: 0, amount: 0 }; row.count += 1; row.amount += Number(order.total || 0); map.set(key, row); });
    return [...map.values()].sort((a, b) => b.count - a.count);
  }, [report.orders]);

  const periodPayouts = useMemo(() => data.payouts.filter((x) => inPeriod(x.created_at, report.since)), [data.payouts, report.since]);
  const periodReturns = useMemo(() => data.returns.filter((x) => inPeriod(x.resolved_at ?? x.created_at, report.since)), [data.returns, report.since]);
  ...
  return <div className="space-y-6">
    ...
    <div className="grid gap-4 lg:grid-cols-3">
      <section className="rounded-2xl border border-border bg-card p-5 lg:col-span-1"><h3 className="mb-4 font-bold">Mənfəət və zərər xülasəsi</h3><div className="space-y-2 text-sm">
        <div className="flex justify-between"><span>Brüt satış dövriyyəsi</span><b>{formatAZN(report.gross)}</b></div>
        <div className="flex justify-between text-red-600"><span>Endirimlər</span><b>−{formatAZN(report.discounts)}</b></div>
        <div className="flex justify-between text-red-600"><span>İstifadə edilən bonus</span><b>−{formatAZN(report.bonusUsed)}</b></div>
        <div className="flex justify-between"><span>Qazanılan bonus öhdəliyi</span><b>{formatAZN(report.bonusEarned)}</b></div>
        <div className="flex justify-between text-red-600"><span>Refund</span><b>−{formatAZN(report.refunds)}</b></div>
        <div className="my-2 border-t" />
        <div className="flex justify-between"><span>Marketplace komissiyası</span><b>{formatAZN(report.marketplace)}</b></div>
        <div className="flex justify-between"><span>Digər xidmət gəlirləri</span><b>{formatAZN(report.registration + report.advertising + report.trends)}</b></div>
        <div className="flex justify-between text-base text-primary"><span className="font-bold">Platforma gəliri</span><b>{formatAZN(report.income)}</b></div>
        <div className="flex justify-between"><span>Orta çek</span><b>{formatAZN(report.averageOrder)}</b></div>
      </div></section>
      <section className="rounded-2xl border border-border bg-card p-5"><h3 className="mb-4 font-bold">Ödəniş üsulları</h3><div className="space-y-3">{paymentMethods.length ? paymentMethods.map((row) => <div key={row.method} className="flex items-center justify-between gap-3 text-sm"><div><b className="uppercase">{row.method}</b><div className="text-xs text-muted-foreground">{row.count} sifariş</div></div><b>{formatAZN(row.amount)}</b></div>) : <div className="py-6 text-center text-sm text-muted-foreground">Məlumat yoxdur</div>}</div></section>
      <section className="rounded-2xl border border-border bg-card p-5"><h3 className="mb-4 font-bold">Sifariş statusları</h3><div className="space-y-3">{orderStatuses.length ? orderStatuses.map((row) => <div key={row.status} className="flex items-center justify-between gap-3 text-sm"><div><b>{row.status}</b><div className="text-xs text-muted-foreground">{row.count} sifariş</div></div><b>{formatAZN(row.amount)}</b></div>) : <div className="py-6 text-center text-sm text-muted-foreground">Məlumat yoxdur</div>}</div></section>
    </div>
    ...
    <section className="rounded-2xl border border-border bg-card p-5"><h3 className="mb-4 font-bold">Tam Epoint əməliyyat reyestri</h3><div className="overflow-x-auto"><table className="w-full min-w-[1120px] text-xs"><thead><tr className="border-b text-left text-muted-foreground"><th className="py-2">Merchant order</th><th>Tarix</th><th>Məbləğ</th><th>Status</th><th>Provider ID</th><th>Bank ID / RRN</th><th>Kart</th><th>Cavab kodu</th><th>Mesaj</th></tr></thead><tbody>{report.epoint.length ? report.epoint.slice(0, 500).map((row) => <tr key={row.id} className="border-b align-top"><td className="py-3 font-mono">{row.merchant_order_id}</td><td>{formatDateTime(row.paid_at || row.created_at)}</td><td className="font-bold">{formatAZN(row.amount)}</td><td><span className={`rounded-full px-2 py-1 ${paymentPaid(row) ? "bg-emerald-500/10 text-emerald-700" : ["error", "server_error", "failed"].includes(row.status) ? "bg-red-500/10 text-red-700" : "bg-amber-500/10 text-amber-700"}`}>{row.status}</span></td><td className="font-mono">{row.provider_transaction_id || "—"}</td><td className="font-mono">{row.bank_transaction_id || row.rrn || "—"}</td><td className="font-mono">{row.card_mask || "—"}</td><td>{row.response_code || row.operation_code || "—"}</td><td className="max-w-[220px] truncate" title={row.message || ""}>{row.message || "—"}</td></tr>) : <tr><td colSpan={9} className="py-8 text-center text-muted-foreground">Epoint əməliyyatı yoxdur</td></tr>}</tbody></table></div></section>

    <section className="rounded-2xl border border-border bg-card p-5"><h3 className="mb-4 font-bold">Satıcı payout reyestri</h3><div className="overflow-x-auto"><table className="w-full min-w-[980px] text-xs"><thead><tr className="border-b text-left text-muted-foreground"><th className="py-2">Satıcı</th><th>Tarix</th><th>Məbləğ</th><th>Status</th><th>Üsul</th><th>Bank</th><th>Hesab sahibi</th><th>Rekvizit</th><th>Admin qeydi</th></tr></thead><tbody>{periodPayouts.length ? periodPayouts.slice(0, 500).map((row) => <tr key={row.id} className="border-b"><td className="py-3 font-semibold">{profileMap[row.seller_id] || row.seller_id?.slice(0, 8)}</td><td>{formatDateTime(row.created_at)}</td><td className="font-bold">{formatAZN(row.amount)}</td><td>{row.status}</td><td className="uppercase">{row.method}</td><td>{row.bank_name || "—"}</td><td>{row.account_holder || "—"}</td><td className="font-mono">{row.iban || row.card_number || "—"}</td><td>{row.admin_note || "—"}</td></tr>) : <tr><td colSpan={9} className="py-8 text-center text-muted-foreground">Payout məlumatı yoxdur</td></tr>}</tbody></table></div></section>

    <section className="rounded-2xl border border-border bg-card p-5"><h3 className="mb-4 font-bold">Refund və qaytarma reyestri</h3><div className="overflow-x-auto"><table className="w-full min-w-[820px] text-xs"><thead><tr className="border-b text-left text-muted-foreground"><th className="py-2">Qaytarma</th><th>Sifariş</th><th>Satıcı</th><th>Səbəb</th><th>Status</th><th>Məbləğ</th><th>Xərci ödəyən</th><th>Tarix</th></tr></thead><tbody>{periodReturns.length ? periodReturns.slice(0, 500).map((row) => <tr key={row.id} className="border-b"><td className="py-3 font-mono">{row.id.slice(0, 8)}</td><td className="font-mono">{row.order_id?.slice(0, 8) || "—"}</td><td>{profileMap[row.seller_id] || row.seller_id?.slice(0, 8) || "—"}</td><td>{row.reason || "—"}</td><td>{row.status}</td><td className="font-bold text-red-600">{formatAZN(row.refund_amount || 0)}</td><td>{row.cost_paid_by || "—"}</td><td>{formatDateTime(row.resolved_at || row.created_at)}</td></tr>) : <tr><td colSpan={8} className="py-8 text-center text-muted-foreground">Refund məlumatı yoxdur</td></tr>}</tbody></table></div></section>
