...
export function PortalAuthForm({
  ...
  const submit = async (e: React.FormEvent) => {
    ...
    if (role === "pvz") {
      if (!data.session) {
        toast.success("Hesab yaradıldı. E-poçtunuzu təsdiqləyib PVZ qeydiyyatını tamamlayın.");
        setBusy(false);
        return;
      }
    ...
    setBusy(false);
    if (!data.session) {
      toast.success("Hesab yaradıldı. Daxil olmaq üçün e-poçtunuzu təsdiqləyin.");
      return;
    }
    ...
    <div className="container mx-auto px-4 py-10 max-w-lg">
      <div className="bg-card border border-border rounded-2xl p-6 md:p-8 shadow-card">
        <div className="flex justify-center mb-2">
          <img src="/brand/eg-lockup-dark.svg" alt="EG Shop" className="h-36 w-40 object-contain" />
        </div>
