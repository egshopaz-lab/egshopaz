...
const categories = [
  ...
  ["TM", "Tikinti və təmir"],
  ["B", "Bağ və həyət"],
  ["İ", "İdman və istirahət"],
  ["H", "Heyvan məhsulları"],
  ["O", "Kitablar və ofis"],
  ["Ə", "Ərzaq məhsulları"],
  ["S", "Sağlamlıq"],
  ["HD", "Hədiyyə və suvenir"],
  ["Z", "Zərgərlik və saatlar"],
  ["OY", "Oyun və hobbi"],
  ["Ç", "Çantalar və aksesuarlar"],
  ["OF", "Ofis və biznes"],
  ["SE", "Smart ev"],
  ["VS", "Velosiped və skuter"],
  ["MT", "Mətbəx texnikası"],
  ["MX", "Məişət texnikası"],
  ["FV", "Foto və video"],
];
...
const fallbackProducts = [
  ...
  { id: "af318359-3b26-4080-ac5b-67c4518d3ac8", name: "kofe", price: 100, old: null, image: "https://ibhmwwdrzgjgwfrvpjht.supabase.co/storage/v1/object/public/product-images/d39654de-946a-4352-b1f1-77b7facd2705/d58b87e3-b1b5-4084-b21f-3545cc816552.jpg", rating: "4.5", reviews: 4, brand: "EG Shop", category: "Ərzaq məhsulları" },
...
function updateSeo(path) {
  ...
  const description = product
    ? `${product.name} - ${money(product.price)}. EG Shop-da s?b?t, sifari? v? onlayn öd?nis.`
    : "EG Shop marketplace: kateqoriyalar, axtar??, sat?c? paneli, s?b?t, sifari? v? onlayn öd?nis.";
...
function routePage(path) {
  ...
  if (path === "/shops") return infoPage("Ma?azalar", "Sat?c?lar v? brend ma?azalar", "Platformada aktiv ma?azalar, reytinql?r v? sat?c? profill?ri bir yerd? göst?rilir.", ["T?sdiqli sat?c? rozeti", "Sat?c? m?hsullar?", "?laq? v? t?hvil m?lumatlari"]);
  if (path === "/compare") return comparePage();
  if (path === "/map") return infoPage("X?rit?", "Çatdirilma v? PVZ nöqt?l?ri", "Müst?ri yax?n t?hvil m?nt?q?sini seçm?k v? sifari?i x?rit?d? izl?m?k imkanina sahib olacaq.", ["PVZ m?nt?q?l?ri", "Kuryer zonalari", "Canli status"]);
  ...
  return `
    <section class="account-page">
      <div class="account-tabs">
        <button class="active" type="button"><span>H</span>Şəxsi kabinet</button>
        <button type="button" data-account-section="orders"><span>S</span>Sifarişlərim</button>
        <button type="button" data-account-section="returns"><span>Q</span>Qaytarmalarım</button>
        <button type="button" data-action="favorites"><span>F</span>Sevimlilər</button>
        <button type="button" data-action="cart"><span>C</span>Səbət</button>
      </div>
      ...
      <div class="account-stat-grid">
        <article><span>S</span><b id="accountOrderCount">0</b><small>Sifarişlərim</small></article>
        <article><span>F</span><b id="accountFavoriteCount">0</b><small>Sevimlilər</small></article>
        <article><span>C</span><b id="accountCartCount">0</b><small>Səbət</small></article>
        <article><span>B</span><b>0</b><small>Bildirişlər</small></article>
      </div>
      ...
      <section class="account-menu-card">
        <button type="button" data-account-section="orders"><span>S</span><b>Sifarişlərim</b><small>Sifariş statusu və izləmə</small><i>›</i></button>
        <button type="button" data-account-section="addresses"><span>U</span><b>Ünvanlarım</b><small>Çatdırılma məlumatları</small><i>›</i></button>
        <button type="button" data-account-section="payments"><span>O</span><b>Ödəniş üsulları</b><small>Ödəniş məlumatları</small><i>›</i></button>
        <button type="button" data-account-section="messages"><span>M</span><b>Mesajlar</b><small>Satıcılarla yazışmalar</small><i>›</i></button>
        <button type="button" data-account-section="reviews"><span>R</span><b>Rəylərim</b><small>Məhsul rəyləri</small><i>›</i></button>
      </section>
    </section>`;
}
...
function sellerDashboardPage() {
  ...
  const nav = [
    ["dashboard", "Dashboard", "D"], ["products", "Məhsullar", "M"], ["categories", "Kateqoriyalar", "K"],
    ["orders", "Sifarişlər", "S"], ["customers", "Müştərilər", "C"], ["analytics", "Analitika", "A"],
    ["finance", "Maliyyə / Wallet", "W"], ["ads", "Reklam Mərkəzi", "R"], ["coupons", "Kuponlar", "%"], ["campaigns", "Endirim kampaniyaları", "E"],
    ["reviews", "Rəylər", "Y"], ["notifications", "Bildirişlər", "B"], ["messages", "Mesajlar", "M"],
    ["settings", "Mağaza ayarları", "A"], ["profile", "Profil", "P"], ["security", "Təhlükəsizlik", "T"], ["logout", "Çıxış", "X"],
  ];
  return `
    <section class="seller-pro-dashboard" data-theme="light">
      ...
      <div class="seller-pro-main">
        <header class="seller-pro-header">
          ...
          <label class="seller-pro-search"><span>A</span><input data-seller-search placeholder="Məhsul, sifariş, müştəri axtar"></label>
          <button type="button" class="seller-icon-btn" data-seller-theme>G</button>
          <button type="button" class="seller-icon-btn" data-seller-section="notifications">B</button>
...
function sellerDashboardView(items, totalRevenue) {
  ...
  return `
    <div class="seller-kpi-grid">
      ${[
        ["Ümumi sat??", money(totalRevenue), "+12%"], ["Bugünkü sat??", money(totalRevenue / 18), "+4%"],
        ["Bu ay?n g?liri", money(totalRevenue / 3), "+18%"], ["Gözl?y?n sifari?l?r", pending, "n?zar?t"],
        ["Tamamlanan sifari?l?r", completed, "stabil"], ["M?hsul say?", items.length, "aktiv"],
        ["Müst?ri sayi", 24, "+6"], ["Wallet balans?", money(totalRevenue * .72), "haz?r"],
    ...
    <div class="seller-chart-grid">
      <article class="seller-card"><div class="seller-card-head"><h2>Sat?? qrafiki</h2><select><option>Günd?lik</option><option>H?ft?lik</option><option>Ayl?q</option></select></div><div class="seller-bars">${[35,55,42,68,80,58,92].map((h) => `<i style="height:${h}%"></i>`).join("")}</div></article>
      <article class="seller-card"><div class="seller-card-head"><h2>G?lir qrafiki</h2><button type="button" data-seller-section="analytics">Detall?</button></div><div class="seller-line-chart"><span></span></div></article>
    </div>
...
function sellerProductsView(items) {
  return `
    <div class="seller-card">
      <div class="seller-card-head"><h2>M?hsul ?lav? et</h2><button type="button" data-panel="seller">Geni? m?hsul paneli</button></div>
      <form class="seller-form-grid" data-seller-save>
        <label>M?hsul ad?<input name="name" required placeholder="M?hsul ad?"></label><label>SKU<input name="sku" placeholder="SKU-001"></label>
        ...
        <label class="wide">Çoxlu ??kil yükl?<input name="images" type="file" accept="image/*" multiple></label>
        <div class="seller-form-actions"><button class="form-secondary" type="button" data-seller-draft>Draft saxla</button><button class="form-submit" type="submit">M?hsulu yay?mla</button></div>
      </form>
    </div>
    <div class="seller-card"><div class="seller-card-head"><h2>M?hsullar</h2><div><input class="seller-table-filter" data-table-filter placeholder="Axtar??"><button type="button">Filter</button></div></div><div class="seller-table">${sellerProductRows(items)}</div><div class="seller-pagination"><button>‹</button><b>1</b><button>›</button></div></div>`;
}
...
function sellerOrdersView(items) {
  ...
  return `<div class="seller-card"><div class="seller-card-head"><h2>Sifari?l?r</h2><input class="seller-table-filter" data-table-filter placeholder="Sifari? axtar"></div><div class="seller-status-tabs">${tabs.map((tab, i) => `<button class="${i === 0 ? "active" : ""}" type="button">${tab}</button>`).join("")}</div><div class="seller-table">${sellerOrderRows(items, true)}</div></div>`;
}

function sellerCustomersView() {
  const customers = ["Elshad Namazov", "Aysel M?mm?dova", "Nigar Aliyeva", "Murad H?s?nli"];
  return `<div class="seller-card"><div class="seller-card-head"><h2>Müst?ril?r</h2><input class="seller-table-filter" data-table-filter placeholder="Müst?ri axtar"></div><div class="seller-table">${customers.map((name, index) => `<div class="seller-table-row"><span class="seller-avatar">${name[0]}</span><span><b>${name}</b><small>${index + 1} sifari? · son sifari? ${index + 2} gün ?vv?l</small></span><b>${money((index + 1) * 120)}</b><button type="button" data-seller-save>Profil</button></div>`).join("")}</div></div>`;
}
...
function sellerAdsCenterView(items, totalRevenue) {
  ...
  return `
    ...
    <div class="seller-chart-grid">
      ...
      <article class="seller-card">
        <div class="seller-card-head"><h2>M?hsulu ?r?li Çixar</h2><button type="button" data-seller-section="products">M?hsullar</button></div>
    ...
    <div class="seller-chart-grid">
      <article class="seller-card">
        <div class="seller-card-head"><h2>Reklam statistikas?</h2><select><option>Günd?lik</option><option>Ayl?q</option></select></div>
        <div class="seller-ad-metrics">
          ${[
            ["Impressions", "18 420"], ["Klik", "842"], ["CTR", "4.57%"], ["Sat??", "38"],
...
function sellerNotificationsView() {
  return `<div class="seller-card"><div class="seller-card-head"><h2>Bildiri?l?r</h2></div>${sellerNotificationsList()}</div>`;
}

function sellerMessagesView() {
  return `<div class="seller-messages"><aside>${["Elshad", "Aysel", "Murad"].map((name, i) => `<button class="${i === 0 ? "active" : ""}" type="button"><b>${name}</b><small>${i + 1} oxunmamis mesaj</small></button>`).join("")}</aside><section><div class="seller-chat"><p>Salam, sifari?im n? vaxt gönd?ril?c?k?</p><p class="me">Salam, sifari? bu gün haz?rlan?r.</p></div><form data-seller-save><input name="message" placeholder="Mesaj yaz"><button class="form-submit">Gönd?r</button><button type="button" class="form-secondary">Fayl</button></form></section></div>`;
}
...
function sellerProfileView(email) {
  return `<div class="seller-card"><div class="seller-card-head"><h2>Profil</h2></div><form class="seller-form-grid" data-seller-save><label>Profil ??kli<input type="file" accept="image/*"></label><label>Email<input value="${email}" disabled></label><label>Ad v? soyad<input placeholder="Sat?c? ad?"></label><label>Telefon<input placeholder="+994 50 000 00 00"></label><label>Yeni ?ifr?<input type="password"></label><label>2FA<select><option>Aktiv deyil</option><option>SMS il? aktiv et</option><option>Authenticator il? aktiv et</option></select></label><button class="form-submit wide">Profili saxla</button></form></div>`;
}

function sellerSecurityView() {
  return `<div class="seller-card"><div class="seller-card-head"><h2>T?hlük?sizlik</h2></div><div class="seller-security-list"><article><b>?ifr? d?yi?</b><button type="button" data-seller-save>Yenil?</button></article><article><b>2FA d?st?yi</b><button type="button" data-seller-save>Aktiv et</button></article><article><b>Aktiv sessiyalar</b><button type="button" data-seller-save>Yoxla</button></article></div></div>`;
}
...
function contactPage() {
  return `<section class="contact-page"><h1>?laq?</h1><div class="contact-card"><p><b>@</b> info@egshop.az</p><p><b>T</b> +994 50 000 00 00</p><p><b>M</b> Bak?, Az?rbaycan</p></div></section>`;
}
...
function catalogPage() {
  ...
  return `
    ...
    <section class="products-section route-products">
      <div class="section-title"><h2>${category || query ? "Filtr n?tic?l?ri" : "Kataloq m?hsullar?"}</h2><button type="button" data-show-all>Ham?s?</button></div>
    ...
    <section class="product-detail-page">
      ...
      <div class="detail-copy">
        ...
        <ul>
          ...
          <li>Sat?c? paneli il? m?hsul idar?etm?si</li>
          <li>T?hlük?siz hesab v? sifari? tarixç?si</li>
        </ul>
      </div>
    </section>
    <section class="products-section route-products">
...
function supportPage() {
  return `
    <section class="route-hero support">
      <span>D?ST?K</span>
      <h1>Al?c?, sat?c? v? PVZ d?st?yi</h1>
  ...
  return `
    <article class="${highlighted ? "lv-card" : "product-card"}" data-product-id="${product.id || ""}" tabindex="0" role="link" aria-label="${product.name} m?hsuluna bax">
      ...
      <div class="${highlighted ? "lv-card-body" : "product-info"}">
        ...
        <button class="${highlighted ? "lv-detail" : "detail-button"}" type="button" data-route="/product/${product.id || ""}">Detall? bax</button>
...
function renderApp() {
  ...
  document.querySelector("#app").innerHTML = `
    <aside class="market-sidebar" aria-label="Əsas menyu">
      ...
      <p>Əsas menyu</p>
      <nav>
        <button type="button" data-route="/"><span>A</span>Ana səhifə</button>
        ...
        <button type="button" data-route="/compare"><span>Q</span>Müqayisə</button>
        ...
        <button type="button" data-route="/support"><span>Y</span>Dəstək</button>
      </nav>
      <div class="side-help">
        <b>Marketplace</b>
    ...
    <header class="site-header">
      <div class="header-inner">
        ...
        <nav class="header-actions" aria-label="İstifadəçi menyusu">
          ...
          <button type="button" data-action="favorites"><span>F</span><b>Sevimlilər</b></button>
          <button type="button" class="basket-action" data-action="cart"><span>C</span><b>Səbət</b><i id="cartCount">0</i></button>
          <button type="button" data-route="${accountRoute}"><span>H</span><b>${userLabel}</b></button>
        </nav>
      </div>
    </header>
    ...
    <main id="top">
      ...
      <section class="lv-product-section lv-win">
        <div class="lv-section-heading"><div><span class="lv-kicker">HƏDİYYƏ</span><h2>Uduşlu məhsullar</h2></div><button type="button" data-show-all>Hamısı</button></div>
    ...
    <nav class="mobile-nav" aria-label="Mobil menyu">
      <button type="button" data-mobile="home"><span>A</span>Ana səhifə</button>
      <button type="button" data-mobile="catalog"><span>K</span>Kataloq</button>
      <button type="button" data-mobile="cart"><span>C</span>Səbət<i id="mobileCartCount">0</i></button>
      <button type="button" data-mobile="favorites"><span>F</span>Sevimlilər</button>
      <button type="button" data-mobile="account"><span>H</span>${userLabel}</button>
    </nav>
...
function bindRoutedControls(root = document) {
  ...
  root.querySelector("[data-forgot-password]")?.addEventListener("click", async () => {
    ...
    try {
      ...
      message.textContent = "?ifr? yenil?m? linki e-poçtunuza gönd?rildi.";
    } catch (error) {
...
function bindSellerDashboard(root = document) {
  ...
  shell.querySelectorAll("[data-seller-toggle]").forEach((button) => button.addEventListener("click", async () => {
    ...
    if (!productId) return notify("M?hsul ID tap?lmad?");
  ...
  try {
    if (view === "products") {
      ...
      await upsertStoreSettings({
        store_name: data.store_name || data["Ma?aza ad?"] || "EG Shop ma?azasi",
    ...
    notify("M?lumat Supabase-d? saxlan?ld?");
    form.reset();
  } catch (error) {
    notify(error.message);
  } finally {
...
function addFooter() {
  ...
  footer.innerHTML = '<b>EG SHOP</b><nav><a href="/about.html">Haqq?m?zda</a><a href="/contact.html">?laq?</a><a href="/privacy.html">M?xfilik siyas?ti</a></nav><small>© 2026 EG Shop</small>';
...
function filterProducts(label) {
  ...
  notify(visible ? `${label}: ${visible} m?hsul tap?ld?` : `${label} üçün hele m?hsul yoxdur`);
...
async function startPayment(address, phone, method) {
  ...
  if (method === "card") {
    ...
    if (!response.ok) throw new Error(result?.message || "Öd?nis baslad?lmad?.");
  ...
  if (!destination) throw new Error(result?.message || "Öd?nis s?hif?si yarad?lamad?.");
...
async function openCart() {
  try {
    const items = await getCart();
    if (!items.length) return showInfo("S?b?t", "<p>S?b?tiniz bo?dur.</p>");
...
function openSellerApplication() {
  ...
  showInfo("Sat?c? ol", `
    ...
    <form id="sellerApplicationForm" class="product-form">
      ...
      <label>??h?r<input name="city" required placeholder="Bak?"></label>
      <label>Ünvan<input name="address" required placeholder="Ma?aza/ofis ünvani"></label>
      ...
      <label>Çatdirilma imkani
        <select name="delivery_type" required>
          ...
          <option>Ma?azadan götürm?</option>
        </select>
      </label>
...
async function openPanel(type) {
  ...
  try {
    ...
    if (!allowed) {
      content.innerHTML = `<span class="dialog-kicker">Giri? m?hduddur</span><h2>${type === "admin" ? "Admin" : "Sat?c?"} paneli</h2><p>Bu hesab üçün uygun rol t?yin edilm?yib.</p>`;
      return;
    }
    if (type === "admin") {
      ...
      content.innerHTML = `
        ...
        <div class="panel-stats">
          <div><b>${admin.profiles.length}</b><span>?stifad??i</span></div>
          <div><b>${admin.products.length}</b><span>M?hsul</span></div>
          <div><b>${admin.orders.length}</b><span>Sifari?</span></div>
          <div><b>${admin.applications.filter((item) => item.status === "pending").length}</b><span>Gözl?y?n sat?c?</span></div>
        </div>
      ...
      content.querySelectorAll("[data-order]").forEach((select) => select.addEventListener("change", async () => {
        ...
        notify("Sifari? statusu yenil?ndi");
      }));
      return;
    }
    ...
    content.innerHTML = `
      <span class="dialog-kicker">Sat?c? paneli</span>
      <h2>Ma?aza idar?etm?si</h2>
      <div class="panel-stats">
        ...
        <div><b>${money(sellerRevenue)}</b><span>Sat?? dövriyy?si</span></div>
      </div>
      <div class="seller-panel-grid">
        <section>
          <h3>Yeni m?hsul yükl?</h3>
      <form id="productForm" class="product-form">
        <p class="flow-hint">M?hsulu ??kil fayli il? yükl?yin v? ya haz?r ??kil URL-i daxil edin.</p>
        <label>M?hsul ad?<input name="name" required></label>
        <label>Kateqoriya
          <select name="category" required>
        ...
        <label>M?hsul ??kli<input name="image_file" type="file" accept="image/*"></label>
        <label>??kil URL<input name="image_url" type="url"></label>
        ...
        <section>
          ...
          <form id="sellerProfileForm" class="product-form compact-form">
            ...
            <label>Ünvan<input name="address" value="${sellerApplication?.address || ""}" placeholder="Ma?aza/ofis ünvani"></label>
          ...
          <h3>Sifari?l?rim</h3>
          <div class="management-list">
            ${sellerOrders.length ? sellerOrders.slice(0, 8).map((item) => `<div><span><b>${item.product_name || item.products?.name || "M?hsul"}</b><small>${item.quantity || 1} ?d?d - ${money(Number(item.price || 0) * Number(item.quantity || 1))} - ${item.orders?.status || "pending"}</small></span></div>`).join("") : "<p>Bu sat?c? üçün hele sifari? yoxdur.</p>"}
          </div>
          <h3>Öd?nis hesabi</h3>
    ...
    content.querySelector("#sellerProfileForm")?.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        notify("Sat?c? profili yenil?ndi");
      } catch (error) {
        notify(error.message);
      } finally {
    ...
    content.innerHTML = `<h2>Ba?lant? x?tas?</h2><p>${error.message}</p>`;
  }
}
...
function openAccountDialog() {
  ...
  if (user) {
    content.innerHTML = `
      ...
      <button class="form-secondary" type="button" id="ordersButton">Sifari?l?rim</button>
    ...
    content.innerHTML = `
      ...
      <h2 id="authTitle">Hesab?n?za daxil olun</h2>
      <div class="auth-tabs" role="tablist">
        <button type="button" class="active" data-auth-mode="login">Giri?</button>
      ...
      <form id="authForm" data-mode="login" novalidate>
        ...
        <div class="register-fields" hidden>
          <label>?ifr?ni t?krarla<input name="password_confirm" type="password" minlength="8" autocomplete="new-password"></label>
          <label class="terms-check"><input name="terms" type="checkbox"> <span>?stifad? ??rtl?rini q?bul edir?m.</span></label>
        </div>
        <button class="auth-link" type="button" data-forgot-password>?ifr?ni unutdum</button>
...
async function handleAuth(event) {
  ...
  try {
    if (mode === "register") {
      if (form.get("password") !== form.get("password_confirm")) throw new Error("?ifr?l?r eyni deyil.");
      if (!form.get("terms")) throw new Error("?stifad? ??rtl?rini q?bul edin.");
      await signUp(form.get("email"), form.get("password"), form.get("full_name"), form.get("phone"));
      message.className = "form-message success";
      message.textContent = "Qeydiyyat tamamland?. E-poçtunuzu t?sdiql?yin.";
      ...
      if (!result?.access_token) throw new Error("Giri? alinmad?. E-poçt v? ?ifr?ni yenid?n yoxlay?n.");
      message.className = "form-message success";
      message.textContent = "Giri? u?urludur. Ana s?hif? açilir...";
      setTimeout(() => window.location.assign("/"), 700);
    }
  } catch (error) {
    message.className = "form-message";
    message.textContent = error?.message || "Giri? alinmad?. E-poçt v? ?ifr?ni yenid?n yoxlay?n.";
  } finally {
    submit.disabled = false;
  }
}
...
async function showOrders() {
  ...
  content.innerHTML = "<h2>Sifari?l?rim</h2>";
  try {
    const orders = await getOrders();
    if (!orders.length) {
      content.innerHTML += "<p>H?l? sifari?iniz yoxdur.</p>";
    } else {
...
function openAccountSection(section) {
  ...
  const panels = {
    ...
    addresses: `
      <div class="account-action-panel">
        <form class="account-form-grid" data-account-demo-form>
          <label>??h?r<input name="city" placeholder="Bak?"></label>
          <label>Ünvan ad?<input name="title" placeholder="Ev, ofis v? ya ma?aza"></label>
    ...
    payments: `
      <div class="account-action-panel">
        ...
        <div class="payment-method-card"><span>?</span><div><b>Nagdsiz öd?nis aktivdir</b><small>S?b?td? “Kartla öd?” seçimi il? sifari? tamamlana bil?r.</small></div></div>
      </div>`,
    messages: `
      <div class="account-action-panel">
        <div class="account-empty compact">Sat?c? il? yazisma sifari? yarad?ldiqdan sonra avtomatik açilir.</div>
        <form class="account-form-grid" data-account-demo-form>
    ...
    reviews: `
      <div class="account-action-panel">
        <div class="account-mini-list">
          <article><b>R?y gözl?y?n m?hsul yoxdur</b><small>Sifari? tamamland?qdan sonra m?hsula r?y yaza bil?c?ksiniz.</small></article>
        </div>
      </div>`,
    returns: `
      <div class="account-action-panel">
        <form class="account-form-grid" data-account-demo-form>
          <label>Sifari? nömr?si<input name="order" placeholder="M?s?l?n: 2033446"></label>
          <label>S?b?b<select name="reason"><option>M?hsul uy?un deyil</option><option>Z?d?li m?hsul</option><option>Yanl?? m?hsul</option><option>Dig?r</option></select></label>
          <label class="wide">?lav? qeyd<textarea name="note" rows="3" placeholder="Qaytarma s?b?bini q?sa izah edin"></textarea></label>
          <button class="form-submit" type="submit">Qaytarma müraci?ti yarat</button>
        </form>
      </div>`,
  };
  showInfo(titles[section] || "??xsi kabinet", panels[section] || `<div class="account-action-panel"><div class="account-empty compact">Bölm? açildi.</div></div>`);
...
function handleRecovery() {
  ...
  content.innerHTML = `
    ...
    <h2>Yeni ?ifr? yarad?n</h2>
    <form id="passwordUpdateForm">
      <label>Yeni ?ifr?<input name="password" type="password" minlength="8" autocomplete="new-password" required></label>
      <label>?ifr?ni t?krarla<input name="confirm" type="password" minlength="8" autocomplete="new-password" required></label>
      <button class="form-submit">?ifr?ni yenil?</button>
  ...
  content.querySelector("form").addEventListener("submit", async (event) => {
    ...
    try {
      if (event.currentTarget.elements.password.value !== event.currentTarget.elements.confirm.value) {
        throw new Error("?ifr?l?r eyni deyil.");
      }
      await updatePassword(event.currentTarget.elements.password.value);
      message.className = "form-message success";
      message.textContent = "?ifr?niz yenil?ndi.";
    } catch (error) {
      message.textContent = error.message;
    }
  });
...
function handlePaymentForm() {
  document.addEventListener("submit", async (event) => {
    ...
    try {
      ...
      notify(error.message || "Öd?nis baslad?lmad?.");
      if (button) {
        button.disabled = false;
...
function handlePaymentResult() {
  ...
  setTimeout(() => {
    notify(payment === "success" ? "Öd?nis q?bul edildi. Sifari? statusu yenil?nir." : "Öd?nis tamamlanmad?.");
  }, 400);
}
